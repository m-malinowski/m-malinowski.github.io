...
layout: post
published: true
title: "Hey AI... what are we actually building?"
---
The best-known use of AI in 2026 is in execution of well-scoped tasks in in the domains of software engineering, general productivity, and mathematics. Within any given company, the human still has the most context about what to build, but the agent does a lot of the building.

But building, maintaining, and managing institutional knowledge is arguably one of the biggest unsolved problems in in any large-scale program or organisation. If you've ever been part of a serious engineering program - think a new quantum computer, or a new AR headset, or a fusion reactor - you probably all to well that keeping hundreds of engineering continuously aligned about what exactly they're supposed to build, on what timescale, and to what specs - is easily half the effort (and it's not the fun half).
...
Rapid knowledge dissemination is always a challenge in R&D work. As new information streams in - simulations that refine previous simplified analyses, lab data that reveals overlooked effects, or customer feedback that reveals neglected pain points - the organisation needs to continuously adapt its goals, timelines, budgets, architecture, and subsystem specs. To that end, enterprise requires processes, people, and time to collect, organise, distill, and share knowledge. This includes a wide range of activities and systems, such as:
...
- Matrix org structures and project assignements
...
One example to illustrate the point: back in 2022, I attended a course on Systems Engineering, where engineering managers - most from automotive, defense, or energy industry - exchange best practices on knowledge management. As I learned then, the definitive software tool used for requirements documentation in the industry is called **IBM Rational Doors**. The UI like this:
...
I think AI is about to make a step change in knowledge management on large engineering projects.

Here are some things that already more-or-less work today - for the organizations that do the necessarry groundwork.
...
4. **AI red-teaming.** AI agents continuously review project artefacts, searching for inconsistencies, poking holes in analyses and assumptions,
...
The starting point is, of course, to connect the agents with the internal knowledge sources. In practice. this tends to be a little more complicated than enabling the google connector to chatgpt, because enterprise knowledge tends to be scattered - especially on projects that started in the pre-AI era. Furthermore, especially for in-person team, a lot of knowledge is simply not available digitally in the first place. The ideal playbook is therefore to:
...
Three months ago - when I wrote the first draft of this post - my view was that one should prioritise code and text as a source of truth. At that time (in my experience), AI was not able to reliably ingest a screenshot of a Gantt chart and anwer questions like "when does project phase 2 end". Thus, my recommendation has been to aggressively move to programmatically generated figures, such that humans can see the image, but AI can read the code.
...
After they have access to the knowledge, AI agents still have a very hard task on their hands. Understandably, figuring out what the project is about just by reading the documentation is tricky, and AI will make lots of mistakes, just like a human would. Thus, it is critical to augment the agents with a memory and a self-improvement loop, such that the agent gets trained up over time.

Suppose you're looking for the the number of people working on project X, but the agent gets confused, tells you the nubmer of people working on project Y. The idea is that you tell it "that's incorrect, I asked about project X, but you gave me numbers for project Y", and the agent makes a note in its memory file "remember that folder A contains information about project X, but folder B contains information about project Y", and then (hopefully) doesn't repeat this again.
...
Mindroom is an amazing agentic harness developed by my talented collegue [Bas Nijholt](https://www.nijho.lt/), and you should check out [his blog](https://www.nijho.lt/post/mindroom/) for more details. Below, I want to show a few high-level features, and how they make our organisation faster.
...
Any feedback I give to the agent gets recorded in its memory, and others in the project benefit from it going forward.
...
Giving each project a separate agent - rather than assigning a global "know it all agent" for all needs - has several benefits, but one of them is that is enforces knowledge access controls. So for example, the finance team could have a finance room, with agent connected to sensitive finance information, that nobody else can access.

The platform contains both shared and personal agents, and (within IonQ mindroom deployment) each agent has its own virtual machine. Thus, one can use Mindroom beyond knowledge management, as a general-purpose AI harness for coding, simulation, learning, procrastinating, etc. In fact, many people will use mindroom over Codex/Claude just because it's pre-configured and cloud-based.

<img src="{{ "/assets/images/posts/ai-knowledge-management/mindroom-general-purpose-agent.png" | relative_url }}" alt="A Mindroom agent responding to a general-purpose image request" style="display: block; width: 70%; max-width: 431px; margin: 0 auto;">

## What comes next?

After the last 9 monts of experimentation, I feel hugely optimistic about the potential of AI tools to crack the problem of knowledge management in large engineering projects. Organizations and programs which digitise knowledge and connect it to AI agents will get a huge edge over legacy competition - and the gains will only compound over time.

My prediction is that in the coming years, most of IC time on large R&D projects will be spent directly interacting with AI agents who don't just implement solutions, but also manage the work in some shape and form.
